 ...
 *
 * The +1 is what turns "tied" into "clinched" — it assumes finishing level is
 * not good enough. MLB no longer plays a Game 163; ties are broken
 * mathematically, head-to-head record first. So a leader that has already won
 * the season series only needs to *tie* to take the race, and its magic number
 * is exactly one lower.
 *
 * `ownsTiebreaker` should only be true once the season series is mathematically
 * decided. Passing it while games remain between the two clubs would claim a
 * tiebreaker that can still be lost.
 */
export function magicNumber(leaderWins, chaserLosses, totalGames = TOTAL_GAMES, ownsTiebreaker = false) {
  const target = ownsTiebreaker ? totalGames : totalGames + 1;
  return Math.max(0, target - leaderWins - chaserLosses);
}
 ...
 */
/**
 * Does the leader own the head-to-head tiebreaker over this chaser?
 *
 * Only true when the series is over and was won. A led-but-unfinished series is
 * deliberately not counted: the tiebreaker is not banked until the last game
 * between the two clubs is played.
 */
export function ownsTiebreaker(series) {
  if (!series) return false;
  const { wins = 0, losses = 0, remaining = 0 } = series;
  return remaining === 0 && wins > losses;
}

/**
 * `seriesByTeam` maps a chaser's team id to the season series against them,
 * as { wins, losses, remaining } from the Brewers' perspective. Omit it and
 * every race falls back to the conservative +1 formula.
 */
export function computeRaces(teams, teamId, totalGames = TOTAL_GAMES, seriesByTeam = {}) {
  ...
  const races = definitions.map((def) => {
    ...
    const series = seriesByTeam[def.chaser.id] ?? null;
    const tiebreaker = ownsTiebreaker(series);
    const magic = magicNumber(me.wins, def.chaser.losses, totalGames, tiebreaker);
    return {
      key: def.key,
      label: def.label,

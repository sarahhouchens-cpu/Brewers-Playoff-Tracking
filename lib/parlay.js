...
/**
 * Per-leg haircut applied when the model is unproven, at full uncertainty.
 *
 * Model error compounds with leg count. If every leg's probability is overstated
 * by a factor f, an n-leg ticket is overstated by f^n — the first night's raw
 * ratio implied f of about 1.15, which is roughly 2x on a five-leg ticket and
 * 1.5x on a three-leg one. Expected value computed on those numbers therefore
 * flatters long tickets specifically, which is exactly backwards: the more legs
 * a ticket has, the less its probability should be trusted.
 *
 * This discounts each leg by an amount that fades as calibration earns
 * confidence, so long tickets are penalised hardest while the model is unproven
 * and the penalty disappears once results justify the projections.
 */
export const MAX_LEG_HAIRCUT = 0.08;

/**
 * Shade a ticket's probability for model uncertainty.
 * `confidence` is the calibration's evidence weight, 0 (unproven) to 1 (proven).
 */
...
/**
 * How many candidates to combine, split by how they earn their place.
 *
 * Ranking the pool purely by edge quietly excluded every long-priced leg. A
 * home run prop rarely has an "under" posted, so it cannot be devigged and its
 * edge is null — and `edge ?? 0` sorts an unmeasured leg below every measured
 * positive one. Unknown is not the same as zero, and the effect was that the
 * pool held only short favourites: no three-leg ticket could reach the $100
 * floor, and the one home run leg the rules allow never appeared.
 *
 * The pool therefore reserves places for the longest prices available, so
 * tickets can be built across the whole range rather than only from favourites.
 */
const CANDIDATE_POOL = { byEdge: 10, byPrice: 4 };
...
export function buildParlays(candidates, options = {}) {
  ...
  const eligible = candidates.filter(isEligibleLeg);
  ...
  const chosenIds = new Set(byEdge.map((l) => `${l.playerId}|${l.market}`));
  const byPrice = [...eligible]
    .filter((l) => !chosenIds.has(`${l.playerId}|${l.market}`))
  ...
  for (let size = LEG_COUNT.min; size <= LEG_COUNT.max; size++) {
    for (const legs of combinations(pool, size)) {
      ...
      const adjusted = confidenceAdjusted(probability, legs.length, confidence);
      tickets.push({
        legs,
        decimal,
        payout: totalReturn,
        ...
        // Ranked on the shaded number so leg count is priced, not rewarded.
        expectedValue: expectedValue(stake, decimal, adjusted),
        rawExpectedValue: expectedValue(stake, decimal, probability),
  ...
  // value can favour long tickets across the board, and a board of five-leg

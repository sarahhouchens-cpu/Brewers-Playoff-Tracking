...
import { americanToDecimal, parlayDecimal, parlayProbability, payout, expectedValue, devig, impliedProbability } from './odds.js';
...
export function isEligibleLeg(leg) {
  ...
  if (!Number.isFinite(Number(leg.americanOdds))) return false;
  return true;
}
...
export function withEdge(leg) {
  ...
  if (!Number.isFinite(Number(leg.oppositeAmericanOdds))) {
    return {
      ...leg,
